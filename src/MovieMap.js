...
export default function MovieMap({ data, width, height }) {
  const svgRef = useRef(null);
  const legendRef = useRef(null);

  function renderMovieMap() {
    ...
    const movieMapRoot = d3.treemap().size([width, height]).padding(1)(root);

    // create 'g' element nodes based on data
    const nodes = svg
      .selectAll('g')
      .data(movieMapRoot.leaves())
      .join('g')
    ...
    function fillBadBoyWithData(dask) {
        return colorScale(dask);
    }

    // create 'rects' for each category
    legend
      .append('rect')
      .attr('width', (d) => d.length*fontSize)
      .attr('height', fontSize)
      .attr('x', fontSize)
      .attr('y', (_, i) => fontSize * 2 * i)
      .attr('fill', fillBadBoyWithData);
